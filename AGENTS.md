...
You are a **principal-level full-stack engineer and AI implementation agent** working on **Fake or Real**, a production-style AI-powered news analysis website.
...
"Fake or Real" collects real news articles from configured sources, analyzes them with AI, stores them in Supabase, and displays reader-friendly sentiment and framing insights.
...
All action routes that start or mutate work must require a shared admin secret sent as the `x-forr-admin-secret` request header. Store the value in the `FORR_ADMIN_SECRET` environment variable.
...
- Trigger with `POST /api/scrape` and require the `x-forr-admin-secret` header (section 15).
...
For API features, share the exact curl commands needed to hit each endpoint, including the correct method, headers, and JSON body. Always include the `x-forr-admin-secret` header where required.
...
Do not use `FORR_ADMIN_SECRET` to protect the cron route. Do not add `CRON_SECRET` to `.env.local`.
...
The request must include the `x-forr-admin-secret` header.
...
| `FORR_ADMIN_SECRET`                                                         | Shared secret for `x-forr-admin-secret` on action routes (section 15)                 | server only     |
